  ...
  const loadData = async () => {
    try {
      ...
      const filteredRecords = user?.role === 'employee'
        ? attRecords.filter(r => r.employeeEmail === user.email)
        : attRecords;
      ...
      if (Array.isArray(empData) && empData.length > 0) {
        setEmployees(
          ...
          })).filter(e => user?.role !== 'employee' || e.email === user.email)
        );
      } else {
        setEmployees([]);
      }
    ...
    <div className="w-full h-full overflow-y-auto bg-slate-50 p-4 md:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span
            className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${serverStatus === "connected"
          ...
          >
            <span
              className={`h-2 w-2 rounded-full ${serverStatus === "connected"
                ? "bg-emerald-500"
                : serverStatus === "offline"
                  ? "bg-rose-500"
                  : "bg-amber-500"
            ...
            <div className="flex items-end gap-[3px] h-24 overflow-hidden">
              {Array.from({ length: 24 }).map((_, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex-1 rounded-full ${isFuture
                      ? "bg-slate-200"
                      : isLate
                        ? "bg-amber-400"
                        : "bg-sky-500"
              ...
              <div className="flex flex-wrap justify-center gap-[3px]">
                {Array.from({ length: 24 }).map((_, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`h-5 w-2 rounded-full ${isActive ? "bg-rose-400" : "bg-slate-200"
                        }`}
                    />
                  );
                })}
                  ...
                  return (
                    <tr key={record.id} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-4 md:px-6 py-3 md:py-4 text-right text-[10px] md:text-xs">
                        {(user?.role === 'hr' || user?.role === 'admin' || record.employeeEmail === user?.email) && (
                          <button
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setCheckIn(new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }))}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setCheckOut(new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }))}
